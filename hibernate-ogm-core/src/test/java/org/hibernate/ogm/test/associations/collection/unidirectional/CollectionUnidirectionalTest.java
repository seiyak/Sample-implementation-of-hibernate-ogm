...
public class CollectionUnidirectionalTest extends OgmTestCase {
	...
	public void testUnidirectionalCollection() throws Exception {
		...
		session.flush();
		assertThat(entityCacheSize(session.getSessionFactory())).isEqualTo(3);
		...
		final SnowFlake removedSf = cloud.getProducedSnowFlakes().iterator()
				.next();
		...
		assertThat(entityCacheSize(session.getSessionFactory())).isEqualTo(4);
		assertThat(associationCacheSize(session.getSessionFactory()))
				.isEqualTo(3);
		...
		assertThat(entityCacheSize(session.getSessionFactory())).isEqualTo(1);
		assertThat(associationCacheSize(session.getSessionFactory()))
				.isEqualTo(0);
