...
public class HashMapTestHelper implements TestableGridDialect {
	...
	private static Map<EntityKey, Map<String, Object>> getEntityMap(
		...
		AbstractMapBasedDatastoreProvider castProvider = getProvider(sessionFactory);
		return castProvider.getEntityMap();
	}

	private static AbstractMapBasedDatastoreProvider getProvider(
			SessionFactory sessionFactory) {
		...
		try {
			if (!(Class.forName(Environment.getProperties().getProperty(
					"hibernate.ogm.datastore.provider")).isInstance(provider))) {
				throw new RuntimeException(
						"Not testing with MapBasedDatastoreProvider, cannot extract underlying map");
			}

			return AbstractMapBasedDatastoreProvider.class.cast(provider);
