...
public class OneToOneTest extends OgmTestCase {
	...
	private static final Logger log = Logger.getLogger(OneToOneTest.class);
	...
	public void testBidirectionalManyToOne() throws Exception {
		...
		assertTrue(husband.getName().equals("Alex"));
		assertNotNull(husband.getWife());
		assertTrue(husband.getWife().getName().equals("Bea"));
		session.clear();
		wife = (Wife) session.get(Wife.class, wife.getId());
		assertNotNull(wife);
		assertTrue(wife.getName().equals("Bea"));
	...
	protected Class<?>[] getAnnotatedClasses() {
		return new Class<?>[] { Horse.class, Cavalier.class, Vehicule.class,
				Wheel.class, Husband.class, Wife.class };
	}
}
