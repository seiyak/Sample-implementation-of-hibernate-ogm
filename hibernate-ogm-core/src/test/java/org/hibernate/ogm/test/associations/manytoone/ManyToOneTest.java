...
public class ManyToOneTest extends OgmTestCase {
	...
	public void testUnidirectionalManyToOne() throws Exception {
		...
		assertThat(entityCacheSize(session.getSessionFactory())).isEqualTo(2);
		assertThat(associationCacheSize(session.getSessionFactory()))
				.isEqualTo(1);
		transaction.commit();
		assertThat(entityCacheSize(session.getSessionFactory())).isEqualTo(2);
		assertThat(associationCacheSize(session.getSessionFactory()))
		...
		session.delete(jug);
		transaction.commit();
		assertThat(entityCacheSize(session.getSessionFactory())).isEqualTo(0);
		assertThat(associationCacheSize(session.getSessionFactory()))
				.isEqualTo(0);
	...
	public void testBiDirManyToOneInsertUpdateFalse() throws Exception {
		...
		assertThat(hoegaarden.getBrewery().getBeers()).hasSize(1).containsOnly(
				hoegaarden);
		...
		assertThat(citron.getBrewery().getBeers()).hasSize(1).containsOnly(
				citron);
	...
	protected Class<?>[] getAnnotatedClasses() {
		return new Class<?>[] { JUG.class, Member.class, SalesForce.class,
				SalesGuy.class, Beer.class, Brewery.class };
	}
}
