...
public final class AssociationKey implements Serializable {
	...
	// column value types do have to be serializable so AssociationKey is
	// serializable
	// should it be a Serializable[] type? It seems to be more pain than
	// anything else
	private final Object[] columnValues;
	private final int hashCode;

	public AssociationKey(String table, String[] columnNames,
			Object[] columnValues) {
		if (columnNames.length != columnValues.length) {
			throw new AssertionFailure(
					"Column names do not match column values");
		}
		this.table = table;
	...
	public String toString() {
		...
		for (int index = 0; index < columnNames.length; index++) {
			sb.append("\n\t").append(columnNames[index]).append(" = '")
					.append(columnValues[index]).append("'");
		}
