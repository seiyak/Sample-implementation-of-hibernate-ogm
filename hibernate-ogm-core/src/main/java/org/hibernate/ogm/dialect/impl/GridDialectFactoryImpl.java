...
public class GridDialectFactoryImpl implements GridDialectFactory {
	...
	private static final Logger log = Logger
			.getLogger(GridDialectFactoryImpl.class);

	public GridDialect buildGridDialect(Map configurationValues,
			ServiceRegistry registry) {
		Object value = configurationValues.get(GRID_DIALECT);

		Class<? extends GridDialect> dialectClass = null;
		if (value == null) {
			dialectClass = registry.getService(DatastoreProvider.class)
					.getDefaultDialect();
		} else if (value instanceof String) {
			Class<?> maybeDialectClass;
			try {
				maybeDialectClass = registry.getService(
			...
			throw new RuntimeException(value.getClass().getCanonicalName());
		}
		try {
			// FIXME not sure I like this constructor business. Argue with Sanne
			// to me that's blocking the doors for future enhancements (ie
			// injecting more things)
			// an alternative is to pass the ServiceRegistry verbatim but I'm
			// not sure that's enough either
			...
			if (injector == null) {
				...
				log.warn("Dialect doesn't have a proper constructor "
						+ dialectClass.getCanonicalName());
			}
			GridDialect gridDialect = (GridDialect) injector
					.newInstance(registry.getService(DatastoreProvider.class));
			// log.useGridDialect(gridDialect.getClass().getName());

			return gridDialect;
		} catch (Exception e) {
			// throw log.cannotInstantiateGridDialect(dialectClass, e);
			throw new RuntimeException("can't instantiate GridDialect "
					+ dialectClass.getCanonicalName());
		}
	}
}
