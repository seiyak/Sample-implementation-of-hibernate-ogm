...
public class OgmCollectionPersister extends AbstractCollectionPersister
	...
	private static final Logger log = Logger
			.getLogger(OgmCollectionPersister.class);
	...
	public OgmCollectionPersister(final Collection collection,
			final CollectionRegionAccessStrategy cacheAccessStrategy,
			final Configuration cfg, final SessionFactoryImplementor factory)
			throws MappingException, CacheException {
		super(collection, cacheAccessStrategy, cfg, factory);
		ServiceRegistry registry = factory.getServiceRegistry();
		final TypeTranslator typeTranslator = registry
				.getService(TypeTranslator.class);
		this.gridDialect = registry.getService(DatastoreServices.class)
				.getGridDialect();
		...
		if (collection.isOneToMany() && getElementPersister() != null
				&& getElementType().isEntityType()) {
			...
			gridTypeOfAssociatedId = typeTranslator
					.getType(identifierOrUniqueKeyType);
		} else if (collection.isOneToMany()) {
			// one to many but not what we expected
			throw new AssertionFailure(
					"Association marked as one to many but has no ManyToOneType: "
							+ collection.getRole());
		} else if (getElementType().isAssociationType()
				&& getElementType().isEntityType()) {
	...
	@Override
	public Object readKey(ResultSet rs, String[] aliases,
	...
	@Override
	public Object readIdentifier(ResultSet rs, String alias,
			SessionImplementor session) throws HibernateException, SQLException {
		final TupleAsMapResultSet resultset = rs
				.unwrap(TupleAsMapResultSet.class);
		final Tuple keyTuple = resultset.getTuple();
	...
	@Override
	public Object readIndex(ResultSet rs, String[] aliases,
			SessionImplementor session) throws HibernateException, SQLException {
		final TupleAsMapResultSet resultset = rs
				.unwrap(TupleAsMapResultSet.class);
		final Tuple keyTuple = resultset.getTuple();
	...
	@Override
	protected CollectionInitializer createSubselectInitializer(
			SubselectFetch subselect, SessionImplementor session) {
	...
	public boolean isCascadeDeleteEnabled() {
		// TODO always false: OGM does not assume cascade delete is supported by
	...
	protected int doUpdateRows(Serializable key,
		...
		PropertyMetadataProvider metadataProvider = new PropertyMetadataProvider()
				.gridDialect(gridDialect).tableName(getTableName()).key(key)
				.keyColumnNames(getKeyColumnNames())
				.keyGridType(getKeyGridType()).session(session);

		while (entries.hasNext()) {
			Object entry = entries.next();
			if (collection.needsUpdating(entry, i, elementType)) {
				// find the matching element
				RowKey assocEntryKey = getTupleKeyForUpdate(key, collection,
						session, i, entry);
				Tuple assocEntryTuple = metadataProvider
						.getCollectionMetadata().get(assocEntryKey);
				if (assocEntryTuple == null) {
					throw new AssertionFailure(
							"Updating a collection tuple that is not present: "
									+ "table {" + getTableName()
									+ "} collectionKey {" + key + "} entry {"
									+ entry + "}");
				}
				// update the matching element
				// FIXME update the associated entity key data
				updateInverseSideOfAssociationNavigation(session,
						assocEntryTuple, Action.REMOVE, assocEntryKey);

				getElementGridType().nullSafeSet(assocEntryTuple,
						collection.getElement(entry), getElementColumnNames(),
						session);

				updateInverseSideOfAssociationNavigation(session,
						assocEntryTuple, Action.ADD, assocEntryKey);
	...
	private void completeTuple(RowKeyAndTuple keyAndTuple,
			PersistentCollection collection, SessionImplementor session,
			Object entry) {
		final Object element = collection.getElement(entry);
		getElementGridType().nullSafeSet(keyAndTuple.tuple, element,
				getElementColumnNames(), session);
	}

	private RowKeyAndTuple createAndPutTupleforInsert(Serializable key,
		...
		if (hasIndex) {
			...
			indexGridType.nullSafeSet(tuple, incrementIndexByBase(index),
					getIndexColumnNames(), session);
			rowKeyBuilder.addColumns(getIndexColumnNames());
		} else {
			// use element as tuple key
			final Object element = collection.getElement(entry);
			getElementGridType().nullSafeSet(tuple, element,
					getElementColumnNames(), session);
		...
		Tuple assocEntryTuple = metadataProvider
				.createAndPutAssociationTuple(result.key);
	...
	private RowKey getTupleKeyForUpdate(Serializable key,
			PersistentCollection collection, SessionImplementor session, int i,
			Object entry) {
		RowKeyBuilder rowKeyBuilder = new RowKeyBuilder()
				.tableName(getTableName());
		Tuple tuple = new Tuple(EmptyTupleSnapshot.SINGLETON);
		if (hasIdentifier) {
			...
			getKeyGridType().nullSafeSet(tuple, key, getKeyColumnNames(),
					session);
			rowKeyBuilder.addColumns(getKeyColumnNames());
			// No need to write to where as we don't do where clauses in OGM :)
			if (hasIndex && !indexContainsFormula) {
				...
				final Object snapshotElement = collection.getSnapshotElement(
						entry, i);
				if (elementIsPureFormula) {
					throw new AssertionFailure(
							"cannot use a formula-based element in the where condition");
				}
				getElementGridType().nullSafeSet(tuple, snapshotElement,
						getElementColumnNames(), session);
				rowKeyBuilder.addColumns(getIndexColumnNames());
			}
		}
	...
	private RowKey getTupleKeyForDelete(Serializable key,
			PersistentCollection collection, SessionImplementor session,
			Object entry, boolean findByIndex) {
		RowKeyBuilder rowKeyBuilder = new RowKeyBuilder()
				.tableName(getTableName());
		Tuple tuple = new Tuple(EmptyTupleSnapshot.SINGLETON);
		if (hasIdentifier) {
	...
	public int getSize(Serializable key, SessionImplementor session) {
		PropertyMetadataProvider metadataProvider = new PropertyMetadataProvider()
				.key(key).tableName(getTableName()).session(session)
				.gridDialect(gridDialect).tableName(getTableName())
				.keyGridType(getKeyGridType())
				.keyColumnNames(getKeyColumnNames());

		final Association collectionMetadata = metadataProvider
				.getCollectionMetadataOrNull();
	...
	public void deleteRows(PersistentCollection collection, Serializable id,
		...
		if (!isInverse && isRowDeleteEnabled()) {
			...
			PropertyMetadataProvider metadataProvider = new PropertyMetadataProvider()
					.gridDialect(gridDialect).tableName(getTableName()).key(id)
					.keyColumnNames(getKeyColumnNames())
					.keyGridType(getKeyGridType()).session(session);
			...
			if (deletes.hasNext()) {
				...
				while (deletes.hasNext()) {
					...
					// find the matching element
					RowKey assocEntryKey = getTupleKeyForDelete(id, collection,
							session, entry, deleteByIndex);
					Tuple assocEntryTuple = metadataProvider
							.getCollectionMetadata().get(assocEntryKey);
					if (assocEntryTuple == null) {
						throw new AssertionFailure(
								"Deleting a collection tuple that is not present: "
										+ "table {" + getTableName()
										+ "} collectionKey {" + id
										+ "} entry {" + entry + "}");
					}
					...
					updateInverseSideOfAssociationNavigation(session,
							assocEntryTuple, Action.REMOVE, assocEntryKey);
					metadataProvider.getCollectionMetadata().remove(
							assocEntryKey);
	...
	public void insertRows(PersistentCollection collection, Serializable id,
		...
		if (!isInverse && isRowInsertEnabled()) {
			...
			if (log.isDebugEnabled()) {
				log.debug("Inserting rows of collection: "
						+ MessageHelper.collectionInfoString(this, id,
								getFactory()));
			}
			...
			while (entries.hasNext()) {
				...
				if (collection.needsInserting(entry, i, elementType)) {
					...
					RowKeyAndTuple keyAndTuple = createAndPutTupleforInsert(id,
							collection, metadataProvider, session, i, entry);
					completeTuple(keyAndTuple, collection, session, entry);
					updateInverseSideOfAssociationNavigation(session,
							keyAndTuple.tuple, Action.ADD, keyAndTuple.key);
	...
	@Override
	public void recreate(PersistentCollection collection, Serializable id,
			SessionImplementor session) throws HibernateException {

		if (!isInverse && isRowInsertEnabled()) {

			if (log.isDebugEnabled()) {
				log.debug("Inserting collection: "
						+ MessageHelper.collectionInfoString(this, id,
								getFactory()));
			}
			...
			if (entries.hasNext()) {
				...
				while (entries.hasNext()) {
					...
					if (collection.entryExists(entry, i)) {
						...
						RowKeyAndTuple keyAndTuple = createAndPutTupleforInsert(
								id, collection, metadataProvider, session, i,
								entry);
						completeTuple(keyAndTuple, collection, session, entry);
						updateInverseSideOfAssociationNavigation(session,
								keyAndTuple.tuple, Action.ADD, keyAndTuple.key);
				...
				metadataProvider.flushToCache();
				if (log.isDebugEnabled()) {
					log.debug("done inserting collection: " + count
							+ " rows inserted");
				}
	...
	private void updateInverseSideOfAssociationNavigation(
			SessionImplementor session, Tuple tuple, Action action,
			RowKey rowKey) {
		if (associationType == AssociationType.EMBEDDED_FK_TO_ENTITY) {
			// update the associated object
			Serializable entityId = (Serializable) gridTypeOfAssociatedId
					.nullSafeGet(tuple, getElementColumnNames(), session, null);
			final EntityKey entityKey = EntityKeyBuilder.fromPersisterId(
					(OgmEntityPersister) getElementPersister(), entityId);
			final Tuple entityTuple = gridDialect.getTuple(entityKey);
			// the entity tuple could already be gone (not 100% sure this can
			// happen but that feels right)
			if (entityTuple == null) {
				return;
			}
			if (action == Action.ADD) {
				// copy all collection tuple entries in the entity tuple as this
				// is the same table essentially
				...
				if (hasIdentifier) {
					throw new AssertionFailure(
							"A true OneToMany with an identifier for the collection: "
									+ getRole());
				}
				if (hasIndex) {
					// nullify the index
					indexGridType.nullSafeSet(entityTuple, null,
							getIndexColumnNames(), session);
				}
				keyGridType.nullSafeSet(entityTuple, null, getKeyColumnNames(),
						session);
			} else {
			...
			Object[] elementColumnValues = LogicalPhysicalConverterHelper
					.getColumnValuesFromResultset(tuple, elementColumnNames);
			PropertyMetadataProvider associationProvider = new PropertyMetadataProvider()
					.gridDialect(gridDialect)
					.keyColumnNames(elementColumnNames)
					.keyColumnValues(elementColumnValues).session(session)
					.tableName(getTableName());

			// TODO what happens when a row should be *updated* ?: I suspect ADD
			// works OK as it's a put()
			if (action == Action.ADD) {
				// FIXME build the key
				Tuple assocTuple = associationProvider
						.createAndPutAssociationTuple(rowKey);
				for (String columnName : tuple.getColumnNames()) {
					assocTuple.put(columnName, tuple.get(columnName));
				}
				associationProvider.getCollectionMetadata().put(rowKey,
						assocTuple);
			} else if (action == Action.REMOVE) {
				// we try and match the whole tuple as it should be on both
	...
	public void remove(Serializable id, SessionImplementor session)
		...
		if (!isInverse && isRowDeleteEnabled()) {
			...
			if (associationType != AssociationType.OTHER) {
				for (RowKey assocEntryKey : metadataProvider
						.getCollectionMetadata().getKeys()) {
					// we unfortunately cannot mass change the update of the
					// associated entity
					updateInverseSideOfAssociationNavigation(
							session,
							metadataProvider.getCollectionMetadata().get(
									assocEntryKey), Action.REMOVE,
							assocEntryKey);
				}
			}
