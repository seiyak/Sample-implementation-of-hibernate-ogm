...
public final class BasicGridExtractor<J> implements GridValueExtractor<J> {
	...
	private static final Logger log = Logger
			.getLogger(BasicGridExtractor.class);

	private final GridTypeDescriptor gridTypeDescriptor;
	private final JavaTypeDescriptor<J> javaTypeDescriptor;

	public BasicGridExtractor(JavaTypeDescriptor<J> javaTypeDescriptor,
	...
	public J extract(final Tuple resultset, final String name) {
		...
		if (result == null) {
			...
			if (log.isTraceEnabled()) {
				...
				// .extractLoggableRepresentation(result), name);

				log.info("found as column "
						+ javaTypeDescriptor
								.extractLoggableRepresentation(result));
			}
			return result;
		}
	}
