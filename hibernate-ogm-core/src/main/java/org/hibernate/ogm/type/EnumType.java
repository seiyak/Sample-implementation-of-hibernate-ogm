 ...
 */
// TODO It would probably be better to implement all of this as a subclass of
// BasicGridType
public class EnumType extends GridTypeDelegatingToCoreType {

	private static final Logger log = Logger.getLogger(EnumType.class);
	...
	@Override
	public Object nullSafeGet(Tuple rs, String[] names,
			SessionImplementor session, Object owner) throws HibernateException {
		if (names.length > 1) {
			throw new NotYetImplementedException(
					"Multi column property not implemented yet");
		}
		return nullSafeGet(rs, names[0], session, owner);
	}

	@Override
	public Object nullSafeGet(Tuple rs, String name,
			SessionImplementor session, Object owner) throws HibernateException {
		final Object object = rs.get(name);
		if (object == null) {
			log.warn("found [null] as column " + name);
			return null;
		} else {
			log.info("found " + object + " as column " + name);
			if (object instanceof Integer) {
				initEnumValues();
				int ordinal = ((Integer) object).intValue();
				if (ordinal < 0 || ordinal >= enumValues.length) {
					throw new IllegalArgumentException(
							"Unknown ordinal value for enum "
									+ coreEnumType.returnedClass() + ": "
									+ ordinal);
				}
				return enumValues[ordinal];
			} else {
				try {
					return Enum.valueOf(coreEnumType.returnedClass(),
							object.toString());
				} catch (IllegalArgumentException iae) {
					throw new IllegalArgumentException(
							"Unknown name value for enum "
									+ coreEnumType.returnedClass() + ": "
									+ name, iae);
				}
			}
	...
	@Override
	public Object hydrate(Tuple rs, String[] names, SessionImplementor session,
			Object owner) throws HibernateException {
		return nullSafeGet(rs, names, session, owner);
	}

	@Override
	public void nullSafeSet(Tuple resultset, Object value, String[] names,
	...
	public void nullSafeSet(Tuple resultset, Object value, String[] names,
		...
		if (value == null) {
			log.warn("binding [null] to parameter " + names[0]);
		} else {
			Object endValue = isOrdinal ? Integer.valueOf(((Enum<?>) value)
					.ordinal()) : ((Enum<?>) value).name();
			log.info("binding " + endValue + " to parameter(s) " + names[0]);
	...
	// in truth we probably only need the types as injected by the metadata
	// binder
