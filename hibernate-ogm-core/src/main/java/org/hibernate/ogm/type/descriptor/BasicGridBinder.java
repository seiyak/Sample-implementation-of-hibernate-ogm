...
public abstract class BasicGridBinder<X> implements GridValueBinder<X> {
	...
	private static final Logger log = Logger.getLogger(BasicGridBinder.class);
	...
	public BasicGridBinder(JavaTypeDescriptor<X> javaDescriptor,
			GridTypeDescriptor gridDescriptor) {
	...
	public void bind(Tuple resultset, X value, String[] names) {
		if (value == null) {
			for (String name : names) {
				...
				log.warn("binding [null] to parameter " + name);
				resultset.put(name, null);
			}
		} else {
			if (log.isTraceEnabled()) {
				// log.tracef("binding [$s] to parameter(s) $s",
				// javaDescriptor.extractLoggableRepresentation(value),
				// Arrays.toString(names));

				log.warn("binding "
						+ javaDescriptor.extractLoggableRepresentation(value)
						+ " to parameter(s) " + Arrays.toString(names));
			}
	 ...
	 */
	protected abstract void doBind(Tuple resultset, X value, String[] names,
