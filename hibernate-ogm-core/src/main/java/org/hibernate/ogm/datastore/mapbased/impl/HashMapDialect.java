...
public class HashMapDialect implements GridDialect {
	...
	private final AbstractMapBasedDatastoreProvider provider;

	public HashMapDialect(AbstractMapBasedDatastoreProvider provider) {
	...
	public LockingStrategy getLockingStrategy(Lockable lockable,
		...
		if (lockMode == LockMode.PESSIMISTIC_FORCE_INCREMENT) {
			return new PessimisticForceIncrementLockingStrategy(lockable,
					lockMode);
			...
			return new OptimisticForceIncrementLockingStrategy(lockable,
					lockMode);
		}
		return new MapPessimisticWriteLockingStrategy(lockable, lockMode);
	}
	...
	public void nextValue(RowKey key, IntegralDataTypeHolder value,
		...
		int nextValue = provider.getSharedAtomicInteger(key, initialValue,
				increment);
