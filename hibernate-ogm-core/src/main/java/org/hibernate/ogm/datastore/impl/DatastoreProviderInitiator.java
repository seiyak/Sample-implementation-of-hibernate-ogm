...
/**
 * Load DatastoreManager from DATASTORE_MANAGER as string, class or
 * DatastoreManager instance. If the property is not set, Infinispan is used by
 * default.
 * 
 * @author Emmanuel Bernard <emmanuel@hibernate.org>
 */
public class DatastoreProviderInitiator implements
		BasicServiceInitiator<DatastoreProvider> {
	public static final String DATASTORE_PROVIDER = "hibernate.ogm.datastore.provider";
	// private static final Log log = LoggerFactory.make();
	private static final Logger log = Logger
			.getLogger(DatastoreProviderInitiator.class);

	public static final DatastoreProviderInitiator INSTANCE = new DatastoreProviderInitiator();

	@Override
	public DatastoreProvider initiateService(Map configurationValues,
			ServiceRegistryImplementor registry) {
		Object managerProperty = configurationValues.get(DATASTORE_PROVIDER);
		Class<?> managerClass = null;
		if (managerProperty instanceof String) {
			try {
				managerClass = registry.getService(ClassLoaderService.class)
						.classForName(managerProperty.toString());
			} catch (Exception e) {
				// throw log.datastoreClassCannotBeFound(managerProperty
				// .toString());
		...
		if (managerClass != null) {
			...
			try {
				return logAndReturn((DatastoreProvider) managerClass
						.newInstance());
			} catch (Exception e) {
				// throw log.unableToInstantiateDatastoreManager(
		...
		if (managerProperty instanceof DatastoreProvider) {
			...
			// throw log.unknownDatastoreManagerType(managerProperty.getClass()
			// .getName());
			throw new RuntimeException();
		}
	}
	...
	private DatastoreProvider logAndReturn(DatastoreProvider datastoreProvider) {
		...
		// throw new RuntimeException();
		log.info(datastoreProvider.getClass().getName());
	...
	private DatastoreProvider guessDatastoreProvider(ClassLoaderService service) {
		Class<?> managerClass = null;
		try {
			managerClass = service
					.classForName("org.hibernate.ogm.datastore.infinispan.impl.InfinispanDatastoreProvider");
		} catch (Exception e) {
