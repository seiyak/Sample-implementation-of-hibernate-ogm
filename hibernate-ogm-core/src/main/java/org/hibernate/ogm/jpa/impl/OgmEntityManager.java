...
/**
 * Delegates most method calls to the underlying EntityManager however, queries
...
public class OgmEntityManager implements EntityManager {
	...
	private static Logger log = Logger.getLogger(OgmEntityManager.class);

	public OgmEntityManager(OgmEntityManagerFactory factory,
			EntityManager hibernateEm) {
	...
	public void persist(Object entity) {
		log.info("Hibernate: persist " + entity.toString());
	...
	public Query createQuery(String qlString) {
		...
		if (qlString != null
				&& qlString.toLowerCase().startsWith("delete from")) {
			// pretend you care
			return new LetThroughExecuteUpdateQuery();
		}
		throw new NotSupportedException("OGM-21",
				"JP-QL queries are not supported yet");
	}

	@Override
	public <T> TypedQuery<T> createQuery(CriteriaQuery<T> criteriaQuery) {
		throw new NotSupportedException("OGM-8",
				"criteria queries are not supported yet");
	}

	@Override
	public <T> TypedQuery<T> createQuery(String qlString, Class<T> resultClass) {
		throw new NotSupportedException("OGM-14",
				"typed queries are not supported yet");
	}

	@Override
	public Query createNamedQuery(String name) {
		throw new NotSupportedException("OGM-15",
				"named queries are not supported yet");
	}
	...
	public <T> T unwrap(Class<T> cls) {
		...
		if (Session.class.isAssignableFrom(cls)
				|| SessionImplementor.class.isAssignableFrom(cls)) {
	...
	private OgmSession buildOgmSession(Session session) {
		final SessionFactory sessionFactory = ((HibernateEntityManagerFactory) hibernateEm
				.getEntityManagerFactory()).getSessionFactory();
		final OgmSessionFactory ogmSessionFactory = new OgmSessionFactory(
