...
/**
 * Transaction implementation using JTA transactions exclusively from the
 * TransactionManager
 * 
 * @author Emmanuel Bernard <emmanuel@hibernate.org>
 */
public class JTATransactionManagerTransaction extends AbstractTransactionImpl
		implements Transaction {

	// private static final Log log = LoggerFactory.make();
	private static final Logger log = Logger
			.getLogger(JTATransactionManagerTransaction.class);
	...
	public JTATransactionManagerTransaction(TransactionCoordinator coordinator) {
		...
		final JtaPlatform jtaPlatform = coordinator.getTransactionContext()
				.getTransactionEnvironment().getJtaPlatform();
	...
	protected void doBegin() {
		...
		try {
			...
			if (newTransaction) {
				...
				isInitiator = true;
				log.info("Began a new JTA transaction");
			}
		} catch (Exception e) {
	...
	protected void afterTransactionBegin() {
		...
		transactionCoordinator().getTransactionContext().afterTransactionBegin(
				this);
	}
	...
	protected void beforeTransactionCommit() {
		...
		final boolean flush = !transactionCoordinator().getTransactionContext()
				.isFlushModeNever()
				&& (isDriver || !transactionCoordinator()
		...
		if (isDriver && isInitiator) {
			transactionCoordinator().getTransactionContext()
					.beforeTransactionCompletion(this);
		}
		...
		if (isDriver) {
			try {
				...
				throw new TransactionException(
						"Unable to determine UserTransaction status", e);
			}
		}
	}
	...
	protected void doRollback() {
		try {
			if (isInitiator) {
				// failed commits automatically rollback the transaction per JTA
				// spec
	...
	private void applyTimeout() {
		try {
			...
			throw new TransactionException("Unable to set timeout: "
					+ getTimeout(), e);
		}
	}
	...
	public JoinStatus getJoinStatus() {
		return JtaStatusHelper.isActive(transactionManager) ? JoinStatus.JOINED
