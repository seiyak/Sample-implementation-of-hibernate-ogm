...
public class OgmLoader implements UniqueEntityLoader {
	...
	private static final Logger log = Logger.getLogger(OgmLoader.class);
	...
	public OgmLoader(OgmCollectionPersister[] collectionPersisters) {
		if (collectionPersisters == null || collectionPersisters.length == 0) {
			throw new AssertionFailure(
					"CollectionPersister[] must not be null or empty");
		}
		...
		this.gridDialect = this.factory.getServiceRegistry()
				.getService(DatastoreServices.class).getGridDialect();
		...
		for (int i = 0; i < collectionPersisters.length; i++) {
			collectionAliases[i] = new OgmColumnCollectionAliases(
	...
	public OgmLoader(OgmEntityPersister[] entityPersisters) {
		if (entityPersisters == null || entityPersisters.length == 0) {
			throw new AssertionFailure(
					"EntityPersister[] must not be null or empty");
		}
	...
	public Object load(Serializable id, Object optionalObject,
		...
		if (log.isDebugEnabled()) {
			log.debug("loading entity: "
					+ MessageHelper.infoString(currentPersister, id,
							currentPersister.getIdentifierType(),
							session.getFactory()));
		}
		QueryParameters qp = new QueryParameters();
		qp.setPositionalParameterTypes(new Type[] { currentPersister
				.getIdentifierType() });
		...
		Object result = doQueryAndInitializeNonLazyCollections(session, qp,
				false);
		return result;
	}
	...
	public final void loadCollection(final SessionImplementor session,
			final Serializable id, final Type type) throws HibernateException {

		if (log.isDebugEnabled()) {
			log.debug("loading collection: "
					+ MessageHelper.collectionInfoString(
							getCollectionPersisters()[0], id, getFactory()));
		}

		Serializable[] ids = new Serializable[] { id };
		QueryParameters qp = new QueryParameters(new Type[] { type }, ids, ids);
		doQueryAndInitializeNonLazyCollections(session, qp, true);
	...
	private Object doQueryAndInitializeNonLazyCollections(
			SessionImplementor session, QueryParameters qp,
		...
		try {
			try {
				result = doQuery(session, qp, returnProxies);
			} finally {
	...
	/**
	 * Execute the physical query and initialize the various entities and
	 * collections
	 */
	private Object doQuery(SessionImplementor session, QueryParameters qp,
			boolean returnProxies) {
		// TODO support lock timeout

		int entitySpan = entityPersisters.length;
		final List<Object> hydratedObjects = entitySpan == 0 ? null
				: new ArrayList<Object>(entitySpan * 10);
		// TODO yuk! Is there a cleaner way to access the id?
		final Serializable id = qp.getOptionalId() != null ? qp.getOptionalId()
				: (Serializable) qp.getCollectionKeys()[0];
		TupleAsMapResultSet resultset = getResultSet(id, session);

		// Todo implement lockmode
		// final LockMode[] lockModesArray = getLockModes(
		...
		try {
			while (resultset.next()) {
				result = getRowFromResultSet(resultset, session, qp,
						// lockmodeArray,
						qp.getOptionalId(), hydratedObjects, keys,
						returnProxies);
			}
		...
		initializeEntitiesAndCollections(hydratedObjects, resultset, session,
				qp.isReadOnly(session));
		// TODO create subselects
		return result;
	}

	/**
	 * If this is a collection initializer, we need to tell the session that a
	 * collection is being initialized, to account for the possibility of the
	 * collection having no elements (hence no rows in the result set).
	 */
	private void handleEmptyCollections(final Serializable[] keys,
			final ResultSet resultSetId, final SessionImplementor session) {
			...
			for (int j = 0; j < collectionPersisters.length; j++) {
				for (int i = 0; i < keys.length; i++) {
					...
					session.getPersistenceContext()
							...
							.getLoadingCollection(collectionPersisters[j],
									keys[i]);
				}
			}
		}
	...
	private Object getRowFromResultSet(ResultSet resultset,
			SessionImplementor session, QueryParameters qp,
			Serializable optionalId, List<Object> hydratedObjects,
			org.hibernate.engine.spi.EntityKey[] keys, boolean returnProxies)
			throws SQLException {
		...
		final Object[] row = getRow(resultset.unwrap(TupleAsMapResultSet.class)
				.getTuple(), persisters, keys, qp.getOptionalObject(),
				getOptionalObjectKey(qp, session),
				getLockModes(qp.getLockOptions()), hydratedObjects, session);
		...
		// applyPostLoadLocks
		// nothing to do atm it seems, the code in Hibernate Core does not do
		// anything either

		return getResultColumnOrRow(row);
	}

	private void extractKeysFromResultSet(SessionImplementor session,
			Serializable optionalId, org.hibernate.engine.spi.EntityKey[] keys) {
		// TODO Implement all Loader#extractKeysFromResultSet (ie resolution in
		// case of composite ids with associations)
		// in the mean time the next two lines are the simplified version
		if (keys.length == 0) {
			// do nothing, this is a collection
		} else {
			final org.hibernate.engine.spi.EntityKey key = session
					.generateEntityKey(optionalId, entityPersisters[0]);
			keys[0] = key;
		}
	}
	...
	private TupleAsMapResultSet getResultSet(Serializable id,
		...
		// TODO this if won't work when we will support collections inside the
		// entity tuple but that will do for now
		final TupleAsMapResultSet resultset = new TupleAsMapResultSet();
		if (getEntityPersisters().length > 0) {
			final EntityKey key = EntityKeyBuilder.fromPersisterId(
					getEntityPersisters()[0], id);
			...
			if (getCollectionPersisters().length != 1) {
				throw new AssertionFailure(
						"Found an unexpected number of collection persisters: "
								+ getCollectionPersisters().length);
			}
			final CollectionPhysicalModel persister = (CollectionPhysicalModel) getCollectionPersisters()[0];
			PropertyMetadataProvider metadataProvider = new PropertyMetadataProvider()
					.gridDialect(gridDialect)
					.tableName(persister.getTableName()).key(id)
					.keyColumnNames(persister.getKeyColumnNames())
					.keyGridType(persister.getKeyGridType()).session(session);
	...
	private Object getResultColumnOrRow(Object[] row) {
		...
		// today we don't use this to apply the result transformer and we don't
		// have operations to do like other loaders
	...
	private void readCollectionElements(Object[] row, ResultSet resultSet,
		...
		if (collectionPersisters != null) {
			...
			// we don't load more than one instance per row, shortcircuiting it
			// for the moment
			...
			for (int i = 0; i < collectionPersisters.length; i++) {
				...
				// true if this is a query and we are loading multiple instances
				// of the same collection role
				// otherwise this is a CollectionInitializer and we are loading
				// up a single collection or batch
				...
				readCollectionElement(owner, key, collectionPersister,
						descriptors[i], resultSet, // TODO CURRENT must use the
													// same instance across all
													// calls
						session);
		...
		if (collectionRowKey != null) {
			...
			if (owner == null) {
				owner = persistenceContext.getCollectionOwner(collectionRowKey,
						persister);
				if (owner == null) {
					// TODO: This is assertion is disabled because there is a
					// bug that means the
					// original owner of a transient, uninitialized collection
					// is not known
					// if the collection is re-referenced by a different object
					// associated
					// with the current Session
					// throw new
					// AssertionFailure("bug loading unowned collection");
				}
			}
	...
	private void initializeEntitiesAndCollections(final List hydratedObjects,
		...
		if (collectionPersisters != null) {
			for (int i = 0; i < collectionPersisters.length; i++) {
				if (collectionPersisters[i].isArray()) {
					// for arrays, we should end the collection load before
					// resolving
					// the entities, since the actual array instances are not
					// instantiated
					// during loading
		...
		if (collectionPersisters != null) {
			for (int i = 0; i < collectionPersisters.length; i++) {
				if (!collectionPersisters[i].isArray()) {
					// for sets, we should end the collection load after
					// resolving
					// the entities, since we might call hashCode() on the
					// elements
					// TODO: or we could do this polymorphically, and have two
					// different operations implemented differently for arrays
					endCollectionLoad(resultSetId, session,
	...
	private void endCollectionLoad(final ResultSet resultSetId,
		...
		// this is a query and we are loading multiple instances of the same
		// collection role
	...
	/**
	 * @param lockOptions
	 *            a collection of lock modes specified dynamically via the Query
	 *            interface
	 */
	protected LockMode[] getLockModes(LockOptions lockOptions) {
		...
		if (lockOptions.getAliasLockCount() == 0
				&& (lockOptions.getLockMode() == null || LockMode.NONE
						.equals(lockOptions.getLockMode()))) {
			return defaultLockModes;
		}
		// FIXME fix the alias issue: could be that the "default" lock mode in
		// LockOptions does that already
		LockMode lockMode = lockOptions
				.getEffectiveLockMode("dummyaliasaswedon'thaveany");
		if (lockMode == null) {
			// NONE, because its the requested lock mode, not the actual!
	 ...
	 */
	private static org.hibernate.engine.spi.EntityKey getOptionalObjectKey(
	 ...
	 * <tt>ResultSet</tt>, etc. Instantiate empty objects to be initialized from
	 * the <tt>ResultSet</tt>. Return an array of objects (a row of results) and
	 * an array of booleans (by side-effect) that determine whether the
	 * corresponding object should be initialized.
	 * 
	 * Copied from Loader#getRow
	 */
	private Object[] getRow(final Tuple resultset,
			...
			final LockMode[] lockModes, final List hydratedObjects,
			final SessionImplementor session) throws HibernateException {
		if (keys.length > 1)
			throw new NotYetImplementedException(
					"Loading involving several entities in one result set is not yet supported in OGM");
		...
		for (int i = 0; i < cols; i++) {
			...
			if (keys[i] == null) {
				...
				if (object != null) {
					...
					instanceAlreadyLoaded(resultset, i, persisters[i], key,
							object, lockModes[i], session);
				} else {
					object = instanceNotYetLoaded(resultset, i,
							persisters[i],
							null, // We don't support rowId
									// descriptors[i].getRowIdAlias(),
							key, lockModes[i], optionalObjectKey,
	 ...
	 */
	private void instanceAlreadyLoaded(final Tuple resultset,
			final int i,
			// TODO create an interface for this usage
			final OgmEntityPersister persister,
			final org.hibernate.engine.spi.EntityKey key, final Object object,
			final LockMode lockMode, final SessionImplementor session)
			throws HibernateException {
		if (!persister.isInstance(object)) {
			throw new WrongClassException("loaded object was of wrong class "
					+ object.getClass(), key.getIdentifier(),
					persister.getEntityName());
		}

		if (LockMode.NONE != lockMode && upgradeLocks()) { // no point doing
															// this if NONE was
															// requested
			...
			if (isVersionCheckNeeded) {
				...
				Object oldVersion = session.getPersistenceContext()
						.getEntry(object).getVersion();
				persister.checkVersionAndRaiseSOSE(key.getIdentifier(),
						oldVersion, session, resultset);
	 ...
	 */
	private Object instanceNotYetLoaded(final Tuple resultset, final int i,
			final Loadable persister, final String rowIdAlias,
			final org.hibernate.engine.spi.EntityKey key,
			final LockMode lockMode,
			final org.hibernate.engine.spi.EntityKey optionalObjectKey,
			final Object optionalObject, final List hydratedObjects,
			final SessionImplementor session) throws HibernateException {
		final String instanceClass = getInstanceClass(resultset, i, persister,
		...
		LockMode acquiredLockMode = lockMode == LockMode.NONE ? LockMode.READ
				: lockMode;
		loadFromResultSet(resultset, i, object, instanceClass, key, rowIdAlias,
	 ...
	 */
	private String getInstanceClass(final Tuple resultset, final int i,
			final Loadable persister, final Serializable id,
			final SessionImplementor session) throws HibernateException {
		// We don't have any discriminator so the class is always the one from
		// the persister
		return persister.getEntityName();
	}

	/**
	 * Hydrate the state an object from the SQL <tt>ResultSet</tt>, into an
	 * array or "hydrated" values (do not resolve associations yet), and pass
	 * the hydrates state to the session.
	 * 
	 * Copied from Loader#loadFromResultSet
	 */
	private void loadFromResultSet(final Tuple resultset, final int i,
			final Object object, final String instanceEntityName,
			final org.hibernate.engine.spi.EntityKey key,
			final String rowIdAlias, final LockMode lockMode,
			final Loadable rootPersister, final SessionImplementor session)
			throws HibernateException {

		final Serializable id = key.getIdentifier();

		// Get the persister for the _subclass_
		final OgmEntityPersister persister = (OgmEntityPersister) getFactory()
				.getEntityPersister(instanceEntityName);

		if (log.isTraceEnabled()) {
			log.trace("Initializing object from ResultSet: "
		...
		final Object[] values = persister.hydrate(resultset, id, object,
				rootPersister,
				// cols,
				eagerPropertyFetch, session);
		...
		if (persister.hasRowId()) {
			throw new HibernateException(
					"Hibernate OGM does nto support row id");
		}
		...
		if (ownerAssociationTypes != null && ownerAssociationTypes[i] != null) {
			...
			if (ukName != null) {
				...
				EntityUniqueKey euk = new EntityUniqueKey(
						rootPersister.getEntityName(), // polymorphism comment
														// above
						ukName,
						type.semiResolve(values[index], session, object), type,
						persister.getEntityMode(), session.getFactory());
	...
	/**
	 * Does this query return objects that might be already cached by the
	 * session, whose lock mode may need upgrading
	 */
	...
	private void registerNonExists(
			final org.hibernate.engine.spi.EntityKey[] keys,
			final Loadable[] persisters, final SessionImplementor session) {
			...
			for (int i = 0; i < keys.length; i++) {
				...
				if (owner > -1) {
					...
					if (keys[i] == null && ownerKey != null) {
						...
						/*
						 * final boolean isPrimaryKey; final boolean
						 * isSpecialOneToOne; if ( ownerAssociationTypes == null
						 * || ownerAssociationTypes[i] == null ) { isPrimaryKey
						 * = true; isSpecialOneToOne = false; } else {
						 * isPrimaryKey =
						 * ownerAssociationTypes[i].getRHSUniqueKeyPropertyName
						 * ()==null; isSpecialOneToOne =
						 * ownerAssociationTypes[i].getLHSPropertyName()!=null;
						 * }
						 */

						// TODO: can we *always* use the "null property"
						// approach for everything?
						/*
						 * if ( isPrimaryKey && !isSpecialOneToOne ) {
						 * persistenceContext.addNonExistantEntityKey( new
						 * EntityKey( ownerKey.getIdentifier(), persisters[i],
						 * session.getEntityMode() ) ); } else if (
						 * isSpecialOneToOne ) {
						 */
						...
						/*
						 * } else {
						 * persistenceContext.addNonExistantEntityUniqueKey( new
						 * EntityUniqueKey( persisters[i].getEntityName(),
						 * ownerAssociationTypes
						 * [i].getRHSUniqueKeyPropertyName(),
	...
	/**
	 * An array of indexes of the entity that owns a one-to-one association to
	 * the entity at the given index (-1 if there is no "owner"). The indexes
	 * contained here are relative to the result of {@link #getEntityPersisters}
	 * .
	 * 
